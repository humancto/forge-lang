...
- [x] 10B.2 `forge search <query>` — search the package index by name/description. Touch: `main.rs`, `package.rs`.
- [x] 10B.3 Checksum verification — verify downloaded package SHA256 matches index entry. Touch: `package.rs`.
...
- [x] 10C.3 Lockfile integrity check — `forge install` verifies `forge.lock` checksums match installed packages, warns on tampering. Touch: `package.rs`.
...
- [x] 11A.3 Intern field names — object field lookups use interned keys, eliminating hash computation on hot paths. Touch: `vm/machine.rs`, `vm/gc.rs`.
